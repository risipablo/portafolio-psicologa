...
import picture from "../ts/pictures"
import { ChevronLeft, ChevronRight, X } from "lucide-react";
import "../../../style/slider.css";

export const SliderMaster = () => {
    ...
    const maxIndex = Math.max(0, picture.length - itemsPerView);
    
    const closeLightbox = useCallback(() => setSelectedImage(null), []);

    const nextImage = (e?: React.MouseEvent) => {
        e?.stopPropagation();
        if (selectedImage !== null) setSelectedImage((selectedImage + 1) % picture.length);
    };

    const prevImage = (e?: React.MouseEvent) => {
        e?.stopPropagation();
        if (selectedImage !== null) setSelectedImage((selectedImage - 1 + picture.length) % picture.length);
    };
    ...
    const lightboxJSX = selectedImage !== null && (
        <div className="lightbox-overlay" onClick={closeLightbox}>
            ...
            <div className="lightbox-content" onClick={(e) => e.stopPropagation()}>
                <img src={picture[selectedImage]} alt="Vista completa" />
                <div className="lightbox-counter">
                    {selectedImage + 1} / {picture.length}
                </div>
            </div>
